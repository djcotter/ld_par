...
#SBATCH --job-name=160928_region_diversity_lowFiltered
...
echo "Ready to perform window analysis using: PAR1, nonPAR, XTR, PAR2..."
...
do
	...
	python scripts/window_calculations_PAR_regions.py temps/filtered_diversity_by_site/${i}_females_filtered_pi_per_site.bed temps/filtered_callable_sites/filtered_called_sites.bed outputs/${i}_females_diversity_${win}.txt
...
touch output/01_README
date > output/01_README
echo "Filter: ${filter_file}" >> output/01_README
echo "Windows: PAR1, nonPAR, XTR, PAR2" >> output/01_README
